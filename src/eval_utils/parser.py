...
def parse_ground_truth(example: Dict[str, Any], data_name: str = "math"):
    if "gt_cot" in example and "gt" in example:
        if data_name in ["math", "math500"] or data_name.startswith("numina_math"):
    ...
    # parse ground truth
    if data_name in ["math", "math500", "minerva_math", "omnimath"] or data_name.startswith(
        "numina_math"
    ):
...
def extract_and_strip(pred_str: str, data_name: str = "math"):
    prediction = extract_answer(pred_str, data_name)
